...
const PopupInputBox = ({ emailInput, setEmailInput, handlePopupSubmission, date, isLoading, setIsLoading }) => {
    ...
    const handleEmailSubmission = async () => {
        const currentDate = new Date();
        currentDate.setHours(0, 0, 0, 0); // Reset hours, minutes, seconds, and milliseconds to compare only dates

        // Convert selectedDate to Date object if it's not already one
    ...
    return (
        <div className="mt-[2rem] md:mt-[0.5rem]  flex md:flex-row flex-col gap-[1rem] items-center justify-center">
            <input
                ref={inputRef}
                value={emailInput}
            ...
            <button onClick={handleEmailSubmission} className="bg-slate-800 no-underline group cursor-pointer relative shadow-2xl shadow-zinc-900 rounded-full p-px text-xs font-semibold leading-6 text-white inline-block">
                ...
                <div className="relative px-[1.5rem] py-[0.6rem] flex space-x-2 items-center justify-center z-10 rounded-full bg-main-red-cta hover:bg-main-red-cta/70 ring-1 ring-white/10 transition-opacity duration-500">
                    <span className="text-[1rem] tracking-wide text-center flex items-center justify-center gap-[1rem] w-[200px] md:w-[150px]">
                        {isLoading ? (
                            <>
                                <span>
                                    Booking
